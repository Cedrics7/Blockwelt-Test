...
export function generateAssets(settings, CONSTANTS, renderer) {
    ...
    const drawCloudNoise = (ctx, s, baseR, baseG, baseB, noiseScale = 3.5, intensity = 40) => {
        for (let x = 0; x < s; x++) {
            for (let y = 0; y < s; y++) {
    ...
    const { texture: dt, dataURL: du } = createCanvasTexture(size, (c, s) => drawCloudNoise(c, s, 110, 80, 50, 4.0, 50));
    generated[BLOCK_TYPES.DIRT] = dt; textureDataURLs[BLOCK_TYPES.DIRT] = du;

    const { texture: grassTop, dataURL: grassTopURL } = createCanvasTexture(size, (c, s) => drawCloudNoise(c, s, 100, 180, 60, 3.0, 30));
    ...
    const { texture: grassSide } = createCanvasTexture(size, (c, s) => {
        drawCloudNoise(c, s, 110, 80, 50, 4.0, 50);
